...
def evaluate_univariate_polynomial_at_x(coeff_list, x):
    """计算一元多项式在x处的取值"""
    result = 0
    n = len(coeff_list)
    for i in range(n):
...
def evaluate_bivariate_polynomial_at_x_y(coeff_matrix, x, y):
    """计算二元多项式在(x,y)的取值"""
    result = 0
    for i in range(len(coeff_matrix)):
        for j in range(len(coeff_matrix[0])):
            result += coeff_matrix[i][j] * (x ** (len(coeff_matrix)-i-1)) * (y ** (len(coeff_matrix[0])-j-1))
    return result

def evaluate_bivariate_polynomial_at_y(coeff_matrix, b):
    """计算一元函数 f(x) = M(x, b)"""

    # 计算多项式的最高次幂
    num_rows = len(coeff_matrix)
    num_cols = len(coeff_matrix[0]) if num_rows > 0 else 0

    # 初始化一元函数的系数列表
    f_x_coeffs = [0] * num_rows

    # 遍历二维系数矩阵并计算一元函数系数
    for i in range(num_rows):
        for j in range(num_cols):
            # 计算 y^j 对应的值
            y_term = b ** (num_cols-1-j)
            # 加入到一元函数系数列表中
            f_x_coeffs[i] += coeff_matrix[i][j] * y_term
...
def bivariate_polynomial_division_x(bivariate_poly, univariate_poly_x):
    """
    对二元多项式除以关于x的一元多项式，返回商多项式和余数多项式。特别地，这里的一元多项式为一元一次多项式
    bivariate_poly 是系数的二维列表，表示二元多项式。
    univariate_poly_x 是系数的一维列表，表示除数多项式关于x。
    """
    degree_x = len(bivariate_poly)
    degree_y = len(bivariate_poly[0])
    constant_C = univariate_poly_x[1]# 一共就两个元素，第一个为1，第二个其实是-tau_x
    quotient = [[0] * degree_y for _ in range(degree_x)]
    remainder = [row[:] for row in bivariate_poly]  # 深拷贝bivariate_poly

    for i in range(degree_x-1):
        print("i+1:",i+1)
        quotient[i+1][:]= remainder[i][:]
        for j in range(degree_y):
            remainder[i+1][j] += (-constant_C) * remainder[i][j]
            remainder[i][j] = 0

    univariate_poly_remainder_y =[remainder[-1][j]  for j in range(degree_y)]
    return quotient, univariate_poly_remainder_y
...
if __name__ == "__main__":
    ...
    # # 示例使用
    # M_coeffs = [
    #     [1, 2],  # M(x, y) = xy + 2x + 3y + 4
    #     [3, 4]
    # ]

    # N_coeffs = [
    #     [5, 6],  # N(x, y) = 5xy + 6x + 7y + 8
    #     [7, 8]
    # ]

    # result_coeffs = bivariate_multiply_polynomials(M_coeffs, N_coeffs)
